...
const QList<arg_option> arguments::opts = {
    arg_option("crlgen",
               ...
               true,
               true,
               "Generate CRL for <ca>. Use the 'name' option to set the "
               "internal name of the new CRL."),
    arg_option("database",
               "database",
               file_argument,
               false,
               false,
               "File name (*.xdb) of the SQLite database or a remote database "
               "descriptor: [user@host/TYPE:dbname#prefix]."),
    arg_option("exit",
               nullptr,
               no_argument,
               false,
    ...
    arg_option(
        "help", nullptr, no_argument, true, false, "Print this help and exit."),
    arg_option("hierarchy",
               "directory",
               file_argument,
               true,
               true,
    ...
    arg_option("issuers",
               ...
               true,
               true,
               "Print all known issuer certificates that have an associated "
               "private key and the CA basic constraints set to 'true'."),
    arg_option(
        "keygen",
        "type",
        required_argument,
        true,
        true,
        "Generate a new key and import it into the database. Use the 'name' "
        "option to set the internal name of the new key. The <type> parameter "
        "has the format: '[RSA|DSA|EC]:[<size>|<curve>]."),
    arg_option("list-curves",
               nullptr,
               no_argument,
               true,
               false,
               "Prints all known Elliptic Curves."),
    arg_option("list-items",
               nullptr,
               no_argument,
               true,
               true,
    ...
    arg_option("name",
               ...
               false,
               true,
               "Provides the name of new generated items. An automatic name "
    ...
    arg_option("no-gui",
               ...
               true,
               false,
               "Do not start the GUI. Alternatively set environment variable "
               "XCA_NO_GUI=1 or call xca as 'xca-console' symlink."),
    arg_option("password",
               "password",
               required_argument,
               false,
    ...
    arg_option("pem",
               ...
               true,
               false,
               "Print PEM representation of provided files. Prints only the "
               "public part of private keys."),
    arg_option("print",
               nullptr,
               no_argument,
               true,
    ...
    arg_option("select",
               ...
               true,
               true,
               "Selects all items in the comma separated id-list to be shown "
               "with 'print', 'text' or 'pem'."),
    arg_option("sqlpass",
               "password",
               required_argument,
               false,
    ...
    arg_option("verbose",
               ...
               false,
               false,
               "Print debug log on stderr. Alternatively set the environment "
               "variable XCA_DEBUG=1."),
    arg_option("version",
               nullptr,
               no_argument,
               true,
...
static QMap<QString, QString> getPassDoc() {
  ...
  passdoc["file:pathname"] =
      "The first line of pathname is the password. If the same pathname "
      "argument is supplied to password and sqlpassword arguments then the "
      "first line will be used for both passwords. pathname need not refer to "
      "a regular file: it could for example refer to a device or named pipe.";
  passdoc["fd:number"] =
...
arg_option::arg_option(
    const char* l, const char* a, int has, bool n, bool nd, const char* h)
...
QString arguments::help() {
  ...
  s += "\n[" COL_CYAN "*" COL_RESET "]" +
       splitQstring(sizeof("[*] ") - 1, width,
                    QString("Needs a database. Either from the commandline or "
                            "as default database")) +
       "\n";
  s += "\n" + splitQstring(0, width,
                           QString("The password options accept the same "
                                   "syntax as openssl does:\n"));
  foreach (QString key, passdoc.keys())
...
int arguments::parse(int argc, char* argv[]) {
  ...
  if (result == ':') {
    result_string =
        QString("Missing option argument for '%1'").arg(argv[optind - 1]);
  }
  if (result == '?') {
...
QString arguments::resultString() const { return result_string; }
...
arguments::arguments(const arguments& a) { *this = a; }
...
arguments::~arguments() { delete[] long_opts; }
...
QStringList arguments::getFiles() const { return files; }

int arguments::getResult() const { return result; }

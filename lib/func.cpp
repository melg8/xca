...
static QString registryInstallDir() {
  ...
  if (RegGetValueW(HKEY_LOCAL_MACHINE, L"Software\\xca", L"Install_Dir64",
                   RRF_RT_REG_SZ, nullptr, inst_dir, &len) != ERROR_SUCCESS)
    return dir;
...
const QString getLibDir() {
  ...
  dirs << ulib + multi + "pkcs11/" << lib + multi + "pkcs11/"
       << ulib + "pkcs11/" << lib + "pkcs11/" << ulib + multi << lib + multi
       << ulib << lib;
...
void migrateOldPaths() {
...
#elif defined(Q_OS_MAC)
  old = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) +
  ...
  foreach (QString n,
           QStringList({"dbhistory", "defaultdb", "defaultlang", ".rnd"})) {
...
#define REG_GUID "MachineGuid"
  ...
  if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, REG_CRYPTO, 0, KEY_READ, &hKey) !=
      ERROR_SUCCESS) {
    XCA_WARN("Registry Key: '" REG_CRYPTO "' not found");
  } else {
    if (RegQueryValueExA(hKey, REG_GUID, nullptr, nullptr, guid, &dwGuid) !=
        ERROR_SUCCESS) {
      XCA_WARN("Registry Key: '" REG_CRYPTO "\\" REG_GUID "' not found");
    }
  }
  RegCloseKey(hKey);
  ...
  snprintf((char*)guid, sizeof guid, "%s",
           CCHAR(QString::fromUtf16(CFStringGetCharactersPtr(uuidCf))));
...
const char* OBJ_ln2sn(const char* ln) { return OBJ_nid2sn(OBJ_ln2nid(ln)); }

const char* OBJ_sn2ln(const char* sn) { return OBJ_nid2ln(OBJ_sn2nid(sn)); }
...
void _openssl_error(const QString& txt, const char* file, int line) {
  ...
  while (int i = ERR_get_error()) {
    ...
    fputs(CCHAR(QString("OpenSSL error (%1:%2) : %3\n")
                    .arg(file)
                    .arg(line)
                    .arg(ERR_error_string(i, nullptr))),
          stderr);
  }
  if (!error.isEmpty()) {
    if (!txt.isEmpty()) {
...
bool _ign_openssl_error(const QString& txt, const char* file, int line) {
  ...
  if (!txt.isEmpty() && ERR_peek_error()) qDebug() << txt;
#else
  (void)txt;
  (void)file;
  (void)line;
...
QString fingerprint(const QByteArray& data, const EVP_MD* type) {
  return formatHash(Digest(data, type), Settings["fp_separator"],
                    Settings["fp_digits"]);
}

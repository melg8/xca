...
db_x509name::db_x509name(const char* classname) : db_base(classname) {}
...
db_x509super::db_x509super(const char* classname) : db_x509name(classname) {
...
dbheaderList db_x509super::getHeaders() {
  ...
  v3nid << NID_subject_alt_name << NID_issuer_alt_name
        ...
        << NID_info_access;
  v3ns_nid << NID_netscape_cert_type << NID_netscape_base_url
           << NID_netscape_revocation_url << NID_netscape_ca_revocation_url
           << NID_netscape_renewal_url << NID_netscape_ca_policy_url
           << NID_netscape_ssl_server_name << NID_netscape_comment;
...
void db_x509super::extractPubkey(QModelIndex index) {
  ...
  key->selfComment(tr("Extracted from %1 '%2'")
                       .arg(pki->getType() == x509 ? tr("Certificate")

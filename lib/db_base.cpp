...
bool db_base::fixedHeaderSize(int sect) { return allHeaders[sect]->size != -1; }
...
void db_base::insertChild(pki_base* child, pki_base* parent) {
  ...
  qDebug() << "insertChild" << *child << "To parent" << *parent << "From"
           << (curr_parent ? QString(*curr_parent) : "NEW") << "COUNT root"
           << rootItem->childCount() << "Count tree" << treeItem->childCount();
}
 ...
 */
void db_base::inToCont(pki_base* pki) { insertChild(pki); }
...
pki_base* db_base::insert(pki_base* item) { return insertPKI(item); }
...
void db_base::dump(const QString& dir) const {
  ...
  if (!d.exists() && !d.mkdir(dirname)) {
    throw errorEx(
        tr("Could not create directory %1").arg(nativeSeparator(dirname)));
  }
...
QModelIndex db_base::index(int row,
                           int column,
...
void db_base::columnResetDefaults() {
  dbheader* hd;
  foreach (hd, allHeaders) { hd->reset(); }
  emit resetHeader();
}
...
void db_base::writeVcalendar(XFile& file, QStringList vcal) const {
  ...
  ics << "BEGIN:VCALENDAR"
      << "VERSION:2.0"
      << "PRODID:-//" XCA_TITLE "//" XCA_VERSION "//" << vcal
      << "END:VCALENDAR";
  file.write(ics.join("\r\n").toUtf8());
}

...
pki_x509req::pki_x509req(const QString& name) : pki_x509super(name) {
...
pki_x509req::pki_x509req(const pki_x509req* req) : pki_x509super(req) {
...
void pki_x509req::restoreSql(const QSqlRecord& rec) {
  ...
  QByteArray ba =
      QByteArray::fromBase64(rec.value(VIEW_x509req_request).toByteArray());
  d2i(ba);
...
void pki_x509req::createReq(pki_key* key,
                            const x509name& dn,
                            const digest& digest,
                            extList el) {
  QList<int> bad_nids;
  bad_nids << NID_authority_key_identifier << NID_issuer_alt_name << NID_undef;
...
QString pki_x509req::getMsg(msg_type msg) const {
  ...
  switch (msg) {
    case msg_import:
      return tr("Successfully imported the %1 certificate request '%2'")
          .arg(type);
    case msg_delete:
      return tr("Delete the %1 certificate request '%2'?").arg(type);
    case msg_create:
      return tr("Successfully created the %1 certificate request '%2'")
          .arg(type);
    /* %1: Number of requests; %2: list of request names */
    case msg_delete_multi:
      return tr("Delete the %1 certificate requests: %2?");
  }
...
void pki_x509req::fload(const QString& fname) {
  ...
  _req =
      PEM_read_bio_X509_REQ(BioByteArray(ba).ro(), nullptr, nullptr, nullptr);
  if (!_req) {
...
int pki_x509req::sigAlg() const { return X509_REQ_get_signature_nid(request); }

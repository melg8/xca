...
void pkcs11::mechanismInfo(const slotid& slot,
                           CK_MECHANISM_TYPE m,
...
QString pkcs11::tokenLogin(QString name, bool so, bool force) {
  ...
  QString text =
      so ? QObject::tr("Please enter the SO PIN (PUK) of the token %1")
...
void pkcs11::setPin(const unsigned char* oldPin,
...
static QString newSoPinTxt =
    QObject::tr("Please enter the new SO PIN (PUK) for the token: '%1'");
static QString newPinTxt =
...
void pkcs11::changePin(const slotid& slot, bool so) {
  ...
  if (PwDialogCore::execute(&p, &newPin, true) == 1) {
    pinp = pin.toLatin1();
    setPin(pinp.constUchar(), pinp.size(), newPin.constUchar(), newPin.size());
  }
  logout();
}
...
void pkcs11::initToken(const slotid& slot,
                       const unsigned char* pin,
                       int pinlen,
                       QString label) {
  CK_RV rv;
...
void pkcs11::storeAttribute(pk11_attribute& attribute,
...
CK_OBJECT_HANDLE pkcs11::createObject(pk11_attlist& attrs) {
  ...
  p11slot.isValid();
  CALL_P11_C(p11slot.lib, C_CreateObject, session, attrs.getAttributes(),
             attrs.length(), &obj);
  if (rv != CKR_OK) {
...
pk11_attr_data pkcs11::generateKey(QString name,
                                   unsigned long mech,
                                   unsigned long bits,
                                   int nid) {
#ifdef OPENSSL_NO_EC
  ...
  pub_atts << label << new_id << pk11_attr_ulong(CKA_CLASS, CKO_PUBLIC_KEY)
  ...
  priv_atts << label << new_id << pk11_attr_ulong(CKA_CLASS, CKO_PRIVATE_KEY)
            << pk11_attr_bool(CKA_TOKEN, true)
  ...
  switch (mech) {
    case CKM_RSA_PKCS_KEY_PAIR_GEN:
      pub_atts << pk11_attr_ulong(CKA_MODULUS_BITS, bits)
    ...
    case CKM_EC_KEY_PAIR_GEN: {
      ...
      EC_GROUP_set_asn1_flag(
          group, info.flags & CKF_EC_NAMEDCURVE ? OPENSSL_EC_NAMED_CURVE : 0);
      ...
      pub_atts << pk11_attr_data(
          CKA_EC_PARAMS, i2d_bytearray(I2D_VOID(i2d_ECPKParameters), group));
      EC_GROUP_free(group);
      break;
    }
#endif
  ...
  CALL_P11_C(p11slot.lib, C_GenerateKeyPair, session, &mechanism,
             ...
             priv_atts.getAttributes(), priv_atts.length(), &pubkey, &privkey);
  if (rv != CKR_OK) {
...
static int rsa_encrypt(int flen,
                       const unsigned char* from,
                       unsigned char* to,
                       RSA* rsa,
                       int padding) {
...
static int rsa_decrypt(int flen,
                       const unsigned char* from,
                       unsigned char* to,
                       RSA* rsa,
                       int padding) {
...
static ECDSA_SIG* ec_do_sign(const unsigned char* dgst,
                             int dgst_len,
...
static EC_KEY_METHOD* setup_ec_key_meth() {
  ...
  EC_KEY_METHOD_set_init(ec_key_meth, ec_init_proc, ec_privdata_free,
                         ec_copy_proc, ec_set_group_proc, ec_set_private_proc,
                         ec_set_public_proc);
  return ec_key_meth;
}
#endif
    ...
    case EVP_PKEY_CTRL_MD:
      ...
      switch (EVP_MD_type((const EVP_MD*)p2)) {
        case NID_ecdsa_with_SHA1:
          qDebug() << __func__ << "NID_ecdsa_with_SHA1 unexpected";
          /* fallthrough */
        case NID_sha1:
        case NID_sha224:
        case NID_sha256:
...
static unsigned char* create_x509_sig(EVP_PKEY_CTX* ctx,
                                      const unsigned char* m,
                                      size_t m_len,
                                      unsigned int* x509_siglen) {
  X509_SIG sig;
  ASN1_TYPE parameter;
  X509_ALGOR algor;
  ...
  *x509_siglen = 0;
  ...
  if (!sig.algor->algorithm) return nullptr;
  if (sig.algor->algorithm->length == 0) return nullptr;
...
static int eng_pmeth_sign_dsa(EVP_PKEY_CTX* ctx,
  ...
  if (EVP_PKEY_type(pkey->type) != EVP_PKEY_DSA) return -1;
  ...
  if (!dsa_sig->s || !dsa_sig->r) goto out;
...
static int eng_pmeth_sign_ec(EVP_PKEY_CTX* ctx,
   ...
   */
  rs_len = len / 2;
  if (!BN_bin2bn(rs_buf, rs_len, ec_sig->r)) goto out;
  if (!BN_bin2bn(rs_buf + rs_len, rs_len, ec_sig->s)) goto out;
  len = i2d_ECDSA_SIG(ec_sig, &sig);
  if (len <= 0) goto out;
  *siglen = len;
  ret = 1;
...
EVP_PKEY* pkcs11::getPrivateKey(EVP_PKEY* pub, CK_OBJECT_HANDLE obj) {
  ...
  if (!e) {
    ...
    if (!p11_rsa_method) {
      ...
      EVP_PKEY_meth_set_sign(p11_rsa_method, nullptr, eng_pmeth_sign_rsa);
      EVP_PKEY_meth_set_ctrl(p11_rsa_method, eng_pmeth_ctrl_rsa, nullptr);
      EVP_PKEY_meth_set_copy(p11_rsa_method, eng_pmeth_copy);
    }
    if (!p11_dsa_method) {
      p11_dsa_method = EVP_PKEY_meth_new(EVP_PKEY_RSA, 0);
      EVP_PKEY_meth_set_sign(p11_dsa_method, nullptr, eng_pmeth_sign_dsa);
      EVP_PKEY_meth_set_ctrl(p11_dsa_method, eng_pmeth_ctrl_dsa, nullptr);
      EVP_PKEY_meth_set_copy(p11_dsa_method, eng_pmeth_copy);
    }
#ifndef OPENSSL_NO_EC
    if (!p11_ec_method) {
      p11_ec_method = EVP_PKEY_meth_new(EVP_PKEY_EC, 0);
      EVP_PKEY_meth_set_sign(p11_ec_method, nullptr, eng_pmeth_sign_ec);
      EVP_PKEY_meth_set_ctrl(p11_ec_method, eng_pmeth_ctrl_ec, nullptr);

...
dbheaderList db_x509req::getHeaders() {
  ...
  h << new dbheader(HD_req_signed, true, tr("Signed"),
    ...
    << new num_dbheader(HD_req_certs, false, tr("Certificate count"),
                        tr("Number of certificates in the database with the "
                           "same public key"));
...
pki_base* db_x509req::insert(pki_base* item) {
  ...
  if (oldreq) {
    XCA_INFO(tr("The certificate signing request already exists in the "
                "database as\n'%1'\nand thus was not stored")
                 .arg(oldreq->getIntName()));
    delete req;
    return nullptr;
  }
...
void db_x509req::newItem() { newItem(nullptr, nullptr); }
...
void db_x509req::newItem(pki_temp* temp, pki_x509req* orig) {
  ...
  try {
    ...
    req->createReq(key, xn, dlg->hashAlgo->current(), dlg->getAllExt());

...
static void myMessageOutput(QtMsgType type,
  ...
  switch (type) {
    ...
    case QtWarningMsg:
      warn_msg = "WARNING";
      severity = COL_LRED "Warning";
      break;
    case QtCriticalMsg:
      warn_msg = "CRITICAL";
      severity = COL_RED "Critical";
      break;
    case QtFatalMsg:
      warn_msg = "FATAL";
      severity = COL_RED "Fatal";
      break;
    case QtInfoMsg:
      severity = COL_CYAN "Info";
      break;
  }
...
static bool compare_pki_base(pki_base* a, pki_base* b) {
  return (a->getSqlItemId().toULongLong() < b->getSqlItemId().toULongLong());
}
...
static void read_cmdline(int argc, char* argv[]) {
  ...
  if (cmd_opts.has("list-items")) {
    ...
    foreach (pki_base* pki, items) {
      list << QString(COL_YELL "%1 " COL_GREEN "%2 " COL_RESET "%3")
  ...
  if (cmd_opts.has("issuers")) {
    ...
    foreach (pki_x509* iss, issuers) {
      ...
      out << QString(COL_YELL "%1 " COL_GREEN "%2 " COL_RESET "%3")
  ...
  if (cmd_opts.has("crlgen")) {
    ...
    foreach (pki_x509* iss, issuers) {
      if (iss->getIntName() == ca || iss->getSqlItemId().toString() == ca) {
        issuer = iss;
        break;
      }
    }
    if (!issuer) {
      XCA_ERROR(QString("Issuer '%1' not found").arg(cmd_opts["crlgen"]));
    } else {
      crljob task(issuer);
...
int main(int argc, char* argv[]) {
  ...
  if (wargv && wargc) {
    int i;
    if (argc != wargc) qWarning() << "argc != wargc" << argc << wargc;
    if (argc > wargc) argc = wargc;
  ...
  foreach (pki_base* pki, pki_base::allitems)
    qDebug() << "Remaining" << pki->getClassName() << pki->getIntName();
  delete mainwin;
  delete gui;
#if defined(Q_OS_WIN32)
  FreeConsole();

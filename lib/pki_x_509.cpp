...
pki_x509::pki_x509(const pki_x509* crt) : pki_x509super(crt) {
  init();
...
pki_x509::pki_x509(const QString& name) : pki_x509super(name) {
  init();
  cert = X509_new();
...
QString pki_x509::getMsg(msg_type msg) const {
  ...
  switch (msg) {
    case msg_import:
      return tr("Successfully imported the certificate '%1'");
    case msg_delete:
      return tr("Delete the certificate '%1'?");
    case msg_create:
      return tr("Successfully created the certificate '%1'");
    /* %1: Number of certs; %2: list of cert names */
    case msg_delete_multi:
      return tr("Delete the %1 certificates: %2?");
  }
...
QSqlError pki_x509::insertSqlData() {
  ...
  SQL_PREPARE(q,
              "INSERT INTO certs (item, hash, iss_hash, serial, issuer, "
              "ca, cert) "
              "VALUES (?, ?, ?, ?, ?, ?, ?)");
  ...
  if (!isCA()) return q.lastError();

  SQL_PREPARE(
      q,
      "INSERT INTO authority (item, template, crlExpire, crlNo, crlDays) "
      "VALUES (?, ?, ?, 0, ?)");
  q.bindValue(0, sqlItemId);
  ...
  q.exec();
  if (fromDataRevList.size() > 0) fromDataRevList.sqlUpdate(sqlItemId);
  return q.lastError();
}
...
void pki_x509::restoreSql(const QSqlRecord& rec) {
  ...
  QByteArray ba =
      QByteArray::fromBase64(rec.value(VIEW_x509_cert).toByteArray());
  d2i(ba);
...
QSqlError pki_x509::deleteSqlData() {
  ...
  QStringList tasks;
  tasks << "DELETE FROM certs WHERE item=?"
  ...
  q = Store.sqlSELECTpki(
      ...
      QList<QVariant>() << QVariant(sqlItemId) << QVariant(sqlItemId));

  while (q.next()) AffectedItems(q.value(0));
...
pki_x509* pki_x509::findIssuer() {
  ...
  if ((issuer = getSigner()) != nullptr) return issuer;
  // first check for self-signed
  if (verify(this)) return this;
...
void pki_x509::fload(const QString& fname) {
  ...
  if (pki_ign_openssl_error() || !_cert) {
    ...
    throw errorEx(tr("Unable to load the certificate in file %1. Tried PEM and "
                     "DER certificate.")
                      .arg(fname));
  }
  X509_free(cert);
...
a1int pki_x509::hashInfo(const EVP_MD* md) const {
  ...
  if (!X509_digest(cert, md, digest, &len)) pki_openssl_error();
  a1int a;
  a.setRaw(digest, len);
  return a;
}
...
void pki_x509::load_token(pkcs11& p11, CK_OBJECT_HANDLE object) {
  ...
  if (type.getValue() != CKC_X_509)
    throw errorEx(
        QString("Unsupported Certificate type %1").arg(type.getValue()));
...
void pki_x509::store_token(bool alwaysSelect) {
  ...
  if (!privkey || !privkey->isToken() || alwaysSelect) {
    if (!p11.selectToken(&slot, nullptr)) return;
  } else {
    card = dynamic_cast<pki_scard*>(privkey);
    if (!card || !card->prepare_card(&slot)) return;
  }

  pk11_attlist p11_atts;
  p11_atts << pk11_attr_ulong(CKA_CLASS, CKO_CERTIFICATE)
  ...
  p11_atts << pk11_attr_bool(CKA_TOKEN, true)
  ...
  if (p11.tokenLogin(p11.tokenInfo().label(), false).isNull()) return;
...
void pki_x509::deleteFromToken() {
  ...
  if (privkey && privkey->isToken()) {
    slotid slot;
    if (!card->prepare_card(&slot)) return;
    p11_slots << slot;
  } else {
    pkcs11 p11;
...
pk11_attlist pki_x509::objectAttributes() {
  pk11_attlist attrs;
  attrs << pk11_attr_ulong(CKA_CLASS, CKO_CERTIFICATE)
        << pk11_attr_ulong(CKA_CERTIFICATE_TYPE, CKC_X_509)
        << pk11_attr_data(CKA_VALUE, i2d());
  return attrs;
}
...
int pki_x509::renameOnToken(const slotid& slot, const QString& name) {
  ...
  tkInfo ti = p11.tokenInfo();
  if (p11.tokenLogin(ti.label(), false).isNull()) return 0;
...
void pki_x509::delSigner(pki_base* s) {
  if (s && (s->getSqlItemId() == issuerSqlId)) issuerSqlId = QVariant();
}
...
bool pki_x509::canSign() const {
  ...
  if (privkey->isToken() && !pkcs11::libraries.loaded()) return false;
  return isCA();
}
...
QString pki_x509::getIndexEntry() {
  ...
  return QString("%1\t%2\t%3\t%4\tunknown\t%5\n")
      .arg(flag, getNotAfter().toPlainUTC(),
           revoked ? revocation.getDate().toPlainUTC() : "", getSerial(),
           QString(X509_NAME_oneline(getSubject().get(), nullptr, 0)));
}

bool pki_x509::pem(BioByteArray& b) { return PEM_write_bio_X509(b, cert); }
...
bool pki_x509::verify(pki_x509* signer) {
  if (getSigner() || !signer) return false;
  if (signer == this && issuerSqlId == sqlItemId && issuerSqlId != QVariant())
    return true;
...
void pki_x509::mergeRevList(x509revList& l) {
  ...
  if (revList.merged) revList.sqlUpdate(sqlItemId);
}
...
bool pki_x509::checkDate() {
  ...
  if (!a.isUndefined() && (a < n)) return false;
  if (b > n) return false;
  pki_openssl_error();
  return true;
}
...
int pki_x509::sigAlg() const { return X509_get_signature_nid(cert); }

pki_x509* pki_x509::getSigner() {
  return Store.lookupPki<pki_x509>(issuerSqlId);
}

bool pki_x509::isRevoked() const { return revocation.isValid(); }

void pki_x509::setRevoked(const x509rev& revok) { revocation = revok; }
...
a1time pki_x509::column_a1time(const dbheader* hd) const {
  switch (hd->id) {
    ...
    case HD_cert_revocation:
      if (isRevoked()) return revocation.getDate();
      break;
    case HD_cert_crl_expire:
      if (canSign()) return crlExpire;
      break;
  }
...
QStringList pki_x509::icsVEVENT_ca() const {
  ...
  QList<pki_crl*> list =
      Store.sqlSELECTpki<pki_crl>("SELECT item FROM crls WHERE issuer = ?",
                                  QList<QVariant>() << QVariant(sqlItemId));

  /* Get latest CRL */
  foreach (pki_crl* pki, list) {
    if (!crl || crl->getNextUpdate() < pki->getNextUpdate()) crl = pki;
  }
  if (crl) ics << crl->icsVEVENT();
...
QVariant pki_x509::getIcon(const dbheader* hd) const {
  int pixnum = 0;
  bool ca;
  QStringList icon_names{":validcertIco", ":validcertkeyIco", ":invalidcertIco",
                         ":invalidcertkeyIco"};
  switch (hd->id) {
    case HD_cert_ca:
...
bool pki_x509::visible() const {
  ...
  if (getIssuerName().search(limitPattern)) return true;
  if (fingerprint(EVP_md5()).contains(limitPattern)) return true;
  if (fingerprint(EVP_sha1()).contains(limitPattern)) return true;
  if (fingerprint(EVP_sha256()).contains(limitPattern)) return true;
  if (getSerial().toHex().contains(limitPattern)) return true;
  return false;
}
...
#define BG_CYAN QBrush(QColor(127, 255, 212))
  ...
  switch (hd->id) {
    ...
    case HD_cert_notAfter: {
      ...
      if (na < now) return QVariant(BG_RED);
      if (certwarn < now) return QVariant(BG_YELLOW);
      break;
    }
    case HD_cert_crl_expire:
      if (canSign()) {
        ...
        if (!crlExpire.isUndefined()) {
          ...
          if (crlex < now) return QVariant(BG_RED);
          if (crlwarn < now || !crlex.isValid()) return QVariant(BG_YELLOW);
        }
      }
  }
  return {};

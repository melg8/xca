...
bool a1time::isUndefined() const { return toTime_t() == 0; }
...
a1time::a1time(const QDateTime& a) : QDateTime(a) { atime = nullptr; }
...
qint64 a1time::age() const { return secsTo(now()); }
...
QString a1time::toFancy() const {
  ...
  if (diff < 2 * SECS_PER_MINUTE) {
    fmt = future ? QObject::tr("in %1 seconds") : QObject::tr("%1 seconds ago");
  } else if (diff < 2 * SECS_PER_HOUR) {
    diff /= SECS_PER_MINUTE;
    fmt = future ? QObject::tr("in %1 minutes") : QObject::tr("%1 minutes ago");
  } else if (dtn == 1) {
    ...
    diff /= SECS_PER_HOUR;
    fmt = future ? QObject::tr("in %1 hours") : QObject::tr("%1 hours ago");
  } else {
...
QString a1time::toPlainUTC() const { return toPlain(UTC_FORMAT); }
...
void a1time::d2i(QByteArray& ba) {
  auto* n = (ASN1_TIME*)d2i_bytearray(D2I_VOID(d2i_ASN1_TIME), ba);
  openssl_error();
  if (n) {
    from_asn1(n);
    ASN1_TIME_free(n);

...
class DbTransaction {
 ...
 public:
  ...
  static bool active() { return mutex > 0; }
  static void addItems(QVariant v) { items << v.toULongLong(); }
  static void setHasTransaction(bool trans) { hasTransaction = trans; }
};

#define Transaction DbTransaction __trans
#define TransBegin() __trans.begin(__FILE__, __LINE__)
#define TransThrow()                                                      \
  if (!__trans.begin(__FILE__, __LINE__)) {                               \
...
class XSqlQuery : public QSqlQuery {
 ...
 public:
  ...
  static void setTablePrefix(QString p) { table_prefix = p; }
  static void clearTablePrefix() { table_prefix.clear(); }

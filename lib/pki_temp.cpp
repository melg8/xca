...
pki_temp::pki_temp(const pki_temp* pk) : pki_x509name(pk) {
  pre_defined = false;
...
pki_temp::pki_temp(const QString& d) : pki_x509name(d) {
  pkiType = tmpl;
  pre_defined = false;

  foreach (QString key, tmpl_keys) { settings[key] = QString(); }
...
void pki_temp::restoreSql(const QSqlRecord& rec) {
  ...
  QByteArray ba =
      QByteArray::fromBase64(rec.value(VIEW_temp_template).toByteArray());
  fromData(ba, version);
}
...
QString pki_temp::getMsg(msg_type msg) const {
  ...
  switch (msg) {
    case msg_import:
      return tr("Successfully imported the XCA template '%1'");
    case msg_delete:
      return tr("Delete the XCA template '%1'?");
    case msg_create:
      return tr("Successfully created the XCA template '%1'");
    /* %1: Number of ktemplates; %2: list of templatenames */
    case msg_delete_multi:
      return tr("Delete the %1 XCA templates: %2?");
  }
...
extList pki_temp::fromCert(pki_x509super* cert_or_req) {
  ...
  settings["nsCertType"] = QString::number((nsCT & 0xf) | ((nsCT & 0xf0) >> 1));

  bool kuCritical;
  settings["keyUse"] =
      QString::number(bitsToInt(el, NID_key_usage, &kuCritical));
...
bool pki_temp::pem(BioByteArray& b) {
  ...
  return PEM_write_bio(b, PEM_STRING_XCA_TEMPLATE, "",
...
void pki_temp::fromExportData(QByteArray data) {
  ...
  fromData((const unsigned char*)data.constData(), data.size(), version);
}
...
QVariant pki_temp::getIcon(const dbheader* hd) const {
  return hd->id == HD_internal_name ? QVariant(QPixmap(":templateIco"))
                                    : QVariant();
}

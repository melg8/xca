...
pki_x509super::pki_x509super(const QString& name) : pki_x509name(name) {}

pki_x509super::pki_x509super(const pki_x509super* x) : pki_x509name(x) {
  keySqlId = x->keySqlId;
}
...
QSqlError pki_x509super::insertSqlData() {
  ...
  XSqlQuery q;
  SQL_PREPARE(q,
              "INSERT INTO x509super (item, subj_hash, pkey, key_hash) "
              "VALUES (?, ?, ?, ?)");
...
pki_x509name::pki_x509name(const QString& name) : pki_base(name) {}

pki_x509name::pki_x509name(const pki_x509name* n) : pki_base(n) {}

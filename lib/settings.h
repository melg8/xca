...
class svalue {
 ...
 public:
  ...
  QStringList split(QString sep) { return get().split(sep); }
  bool empty() { return get().isEmpty(); }
  ...
  operator int() { return get().toInt(); }
  operator unsigned() { return get().toUInt(); }
  operator bool() { return get() == "yes"; }
  operator enum Qt::CheckState() {
    return get() == "yes" ? Qt::Checked : Qt::Unchecked;
  }
  QString operator+(const QString& other) { return get() + other; }
  QString operator+(const char* other) { return get() + other; }
};

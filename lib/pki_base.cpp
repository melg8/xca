...
QString pki_base::comboText() const { return desc; }
...
int pki_base::renameOnToken(const slotid&, const QString&) { return 0; }
...
void pki_base::clear() { childItems.clear(); }
...
const char* pki_base::getClassName() const { return typeid(*this).name(); }
...
void pki_base::setParent(pki_base* p) { parent = p; }
...
void pki_base::takeChild(pki_base* pki) { childItems.removeOne(pki); }
...
pki_base* pki_base::takeFirst() { return childItems.takeFirst(); }
...
QString pki_base::pki_source_name() const {
  switch (pkiSource) {
    ...
    case generated:
      return tr("Generated");
    case transformed:
      return tr("Transformed");
    case token:
      return tr("Token");
    case legacy_db:
      return tr("Legacy Database");
    case renewed:
      return tr("Renewed");
  }
}
...
unsigned pki_base::hash() const { return hash(i2d()); }
...
QStringList pki_base::icsVEVENT(const a1time& expires,
  ...
  return QStringList()
         ...
         << QString("UID:EXP-%1@xca.ovh").arg(uniqueid) << "STATUS:CONFIRMED"
         << QString("DTSTART:%1").arg(expires.toString("yyyyMMdd"))
         << "DURATION:P1D" << QString("SUMMARY:%1").arg(icsValue(summary))
         << QString("DESCRIPTION:%1").arg(full_description) << "BEGIN:VALARM"
         << "ACTION:DISPLAY" << QString("SUMMARY:%1").arg(icsValue(summary))
         << QString("DESCRIPTION:%1").arg(full_description)
         << QString("TRIGGER:-P%1").arg(alarm) << "END:VALARM"
         << "END:VEVENT";
}

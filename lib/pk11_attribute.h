...
class pk11_attribute {
 ...
 public:
  ...
  const CK_ATTRIBUTE* getAttribute() const { return &attr; }
  unsigned long type() const { return attr.type; }
  ...
  bool cmp(const pk11_attribute& other) const {
    return (attr.type == other.attr.type) &&
           ...
           !memcmp(attr.pValue, other.attr.pValue, attr.ulValueLen);
  }
  bool operator==(const pk11_attribute& other) const { return cmp(other); }
};
...
class pk11_attr_bool : public pk11_attribute {
 ...
 public:
  pk11_attr_bool(unsigned long type, bool v = false) : pk11_attribute(type) {
    attr.pValue = &value;
    attr.ulValueLen = sizeof(value);
    setValue(v);
  }
  pk11_attr_bool(const pk11_attr_bool& p) : pk11_attribute(p.type()) {
    attr.pValue = &value;
    attr.ulValueLen = sizeof(value);
    setValue(p.value);
  }
  bool getValue() const { return value ? true : false; }
  void setValue(unsigned long v) { value = v ? 1 : 0; }
};
...
class pk11_attr_ulong : public pk11_attribute {
 ...
 public:
  ...
  pk11_attr_ulong(const pk11_attr_ulong& p) : pk11_attribute(p.type()) {
    attr.pValue = &value;
    attr.ulValueLen = sizeof(value);
    setValue(p.value);
  }
...
class pk11_attr_data final : public pk11_attribute {
 public:
  ...
  pk11_attr_data(unsigned long type,
                 const unsigned char* v = nullptr,
                 unsigned long len = 0)
      : pk11_attribute(type) {
    setValue(v, len);
  }
  pk11_attr_data(const pk11_attr_data& p) : pk11_attribute(p.type()) {
  ...
  pk11_attr_data(unsigned long type, const BIGNUM* bn) : pk11_attribute(type) {
    setConstBignum(bn);
  }
...
class pk11_attlist {
 ...
 public:
  ...
  CK_ATTRIBUTE* getAttributes() { return attributes; }
  unsigned long length() { return attlen; }

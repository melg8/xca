...
pki_key::pki_key(const QString& name) : pki_base(name) {
  key = EVP_PKEY_new();
  key_size = 0;
  isPub = true;
  useCount = -1;
...
pki_key::pki_key(const pki_key* pk) : pki_base(pk) {
  if (pk->key) {
...
void pki_key::write_SSH2_ed25519_private(BIO* b,
  ...
  PEM_write_bio(b, PEM_STRING_OPENSSH_KEY, "", (unsigned char*)(data.data()),
                data.size());
  pki_openssl_error();
#else
  (void)b;
...
void pki_key::writeSSH2private(XFile& file, pem_password_cb* cb) const {
  (void)cb;
  //	pass_info p(XCA_TITLE, tr("Please enter the password protecting the SSH2
  // private key '%1'").arg(getIntName()));
 ...
 */
QString pki_key::getKeyTypeString() const { return keytype::byPKEY(key).name; }

QString pki_key::getTypeString() const { return getKeyTypeString(); }
...
QString pki_key::getMsg(msg_type msg) const {
  ...
  if (isPubKey()) {
    switch (msg) {
      case msg_import:
        return tr("Successfully imported the %1 public key '%2'").arg(ktype);
      case msg_delete:
        return tr("Delete the %1 public key '%2'?").arg(ktype);
      default:
        break;
    }
  } else {
    switch (msg) {
      case msg_import:
        return tr("Successfully imported the %1 private key '%2'").arg(ktype);
      case msg_delete:
        return tr("Delete the %1 private key '%2'?").arg(ktype);
      case msg_create:
        return tr("Successfully created the %1 private key '%2'").arg(ktype);
      default:
        break;
    }
  }
...
bool pki_key::isToken() { return false; }
...
int pki_key::getKeyType() const { return EVP_PKEY_id(key); }
...
static QByteArray ed25519Key(int (*EVP_PKEY_get_raw)(const EVP_PKEY*,
                                                     unsigned char*,
                                                     size_t*),
                             const EVP_PKEY* pkey) {
...
QByteArray pki_key::ed25519PubKey() const { return QByteArray(); }
...
QList<int> pki_key::possibleHashNids() {
  ...
  switch (EVP_PKEY_type(getKeyType())) {
    case EVP_PKEY_RSA:
      nids << NID_md5 << NID_sha1 << NID_sha224 << NID_sha256 << NID_sha384
    ...
    case EVP_PKEY_DSA:
      ...
      break;
    case EVP_PKEY_EC:
      nids << NID_sha1 << NID_sha224 << NID_sha256 << NID_sha384 << NID_sha512;
      break;
  }
  return nids;
}
...
QSqlError pki_key::insertSqlData() {
  ...
  SQL_PREPARE(q,
              "SELECT item FROM x509super WHERE key_hash=? AND "
              "pkey IS NULL");
  ...
  while (q.next()) {
    ...
    if (!x) {
      qDebug("X509 Super class with id %d not found", q.value(0).toInt());
      continue;
    }
  ...
  SQL_PREPARE(q,
              "INSERT INTO public_keys (item, type, hash, len, \"public\") "
              "VALUES (?, ?, ?, ?, ?)");
  q.bindValue(0, sqlItemId);
...
void pki_key::restoreSql(const QSqlRecord& rec) {
  ...
  QByteArray ba =
      QByteArray::fromBase64(rec.value(VIEW_public_keys_public).toByteArray());
  d2i(ba);
...
EVP_PKEY* pki_key::load_ssh2_key(const QByteArray& b) {
  ...
  if (sl[0].startsWith("ssh-rsa")) {
    ...
    DSA_set0_key(dsa, pubkey, nullptr);
    ...
    EC_KEY_set_public_key(
        ec, EC_POINT_bn2point(EC_KEY_get0_group(ec), bn, nullptr, nullptr));
    BN_free(bn);
    pki_openssl_error();
    ...
    pk = EVP_PKEY_new_raw_public_key(EVP_PKEY_ED25519, nullptr,
                                     (const unsigned char*)pub.constData(),
                                     pub.count());
    pki_openssl_error();
#endif
#endif
...
bool pki_key::verify(EVP_PKEY* pkey) const {
  bool verify = true;
  const BIGNUM* a = nullptr;
  const BIGNUM* b = nullptr;
  const BIGNUM* c = nullptr;

  switch (EVP_PKEY_type(EVP_PKEY_id(pkey))) {
    case EVP_PKEY_RSA:
      RSA_get0_key(EVP_PKEY_get0_RSA(pkey), &a, &b, nullptr);
      verify = a && b;
      break;
    case EVP_PKEY_DSA:
    ...
    case EVP_PKEY_ED25519: {
      size_t len;
      verify = EVP_PKEY_get_raw_private_key(pkey, nullptr, &len) == 1 &&
               len == ED25519_KEYLEN;
      break;
    }
#endif
...
bool pki_key::verify_priv(EVP_PKEY*) const { return true; }
...
QByteArray pki_key::X509_PUBKEY_public_key() const {
  X509_PUBKEY* pk = nullptr;
  const unsigned char* p;
  int len;

  X509_PUBKEY_set(&pk, key);
  X509_PUBKEY_get0_param(nullptr, &p, &len, nullptr, pk);
...
void pki_key::print(BioByteArray& bba, enum print_opt opt) const {
  ...
  switch (opt) {
    case print_openssl_txt:
      EVP_PKEY_print_public(bba, key, 0, nullptr);
      break;
    case print_pem:

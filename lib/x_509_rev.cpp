...
QString x509rev::getReason() const { return crl_reasons[reason_idx].lname; }
...
void x509rev::fromREVOKED(const X509_REVOKED* rev) {
  ...
  reason = (ASN1_ENUMERATED*)X509_REVOKED_get_ext_d2i(rev, NID_crl_reason, &j,
                                                      nullptr);
  openssl_error();
  reason_idx = 0;
  if (reason) {
  ...
  at = (ASN1_TIME*)X509_REVOKED_get_ext_d2i(rev, NID_invalidity_date, &j,
                                            nullptr);
  openssl_error();
  if (at) {
    ivalDate = a1time(at);
...
bool x509rev::identical(const x509rev& x) const {
  return serial == x.serial && date == x.date && ivalDate == x.ivalDate &&
         reason_idx == x.reason_idx;
}
...
x509rev::operator QString() const {
  return QString("Rev: %1 D:%2 I:%3 Reason: %4 '%5'\n")
      ...
      .arg(reason_idx)
      .arg(crl_reasons[reason_idx].lname);
}
...
x509revList x509revList::fromSql(QVariant caId) {
  ...
  SQL_PREPARE(q,
              "SELECT serial, date, invaldate, crlNo, reasonBit "

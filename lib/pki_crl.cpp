...
pki_crl::pki_crl(const QString name) : pki_x509name(name) {
  crl = X509_CRL_new();
  pki_openssl_error();
  pkiType = revocation;
}
...
QString pki_crl::getMsg(msg_type msg) const {
  ...
  switch (msg) {
    case msg_import:
      return tr("Successfully imported the revocation list '%1'");
    case msg_delete:
      return tr("Delete the revocation list '%1'?");
    case msg_create:
      return tr("Successfully created the revocation list '%1'");
    /* %1: Number of CRLs; %2: list of CRL names */
    case msg_delete_multi:
      return tr("Delete the %1 revocation lists: %2?");
  }
...
void pki_crl::restoreSql(const QSqlRecord& rec) {
  ...
  QByteArray ba =
      QByteArray::fromBase64(rec.value(VIEW_crls_crl).toByteArray());
  d2i(ba);
...
void pki_crl::fload(const QString& fname) {
  ...
  _crl =
      PEM_read_bio_X509_CRL(BioByteArray(ba).ro(), nullptr, nullptr, nullptr);
  if (!_crl) {
...
QVariant pki_crl::getIcon(const dbheader* hd) const {
  return hd->id == HD_internal_name ? QVariant(QPixmap(":crlIco")) : QVariant();
}

QStringList pki_crl::icsVEVENT() const {
  pki_x509* ca = getIssuer();
  if (ca) {
    return pki_base::icsVEVENT(
        getNextUpdate(), tr("CRL Renewal of CA '%1' due").arg(ca->getIntName()),
  ...
  return pki_base::icsVEVENT(
      getNextUpdate(), tr("Renew CRL: %1").arg(getIntName()),

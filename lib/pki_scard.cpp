...
pki_scard::pki_scard(const QString name) : pki_key(name) { init(); }
...
QString pki_scard::getMsg(msg_type msg) const {
  ...
  switch (msg) {
    case msg_import:
      return tr("Successfully imported the token key '%1'");
    case msg_delete:
      return tr("Delete the token key '%1'?");
    case msg_create:
      return tr("Successfully created the token key '%1'");
...
void pki_scard::restoreSql(const QSqlRecord& rec) {
  ...
  card_manufacturer = rec.value(VIEW_tokens_card_manufacturer).toString();
  isPub = false;
  qDebug() << card_manufacturer << card_serial << card_model << card_label
...
EVP_PKEY* pki_scard::load_pubkey(pkcs11& p11, CK_OBJECT_HANDLE object) const {
  ...
  switch (keytype) {
    ...
    case CKK_EC: {
      ...
      ba = grp.getData();
      group = (EC_GROUP*)d2i_bytearray(D2I_VOID(d2i_ECPKParameters), ba);
      pki_openssl_error();
      ...
      ba = pt.getData();
      os = (ASN1_OCTET_STRING*)d2i_bytearray(D2I_VOID(d2i_ASN1_OCTET_STRING),
                                             ba);
      pki_openssl_error();
...
pk11_attlist pki_scard::objectAttributesNoId(EVP_PKEY* pk, bool priv) const {
  ...
  pk11_attlist attrs(
      pk11_attr_ulong(CKA_CLASS, priv ? CKO_PRIVATE_KEY : CKO_PUBLIC_KEY));
  ...
  switch (EVP_PKEY_type(EVP_PKEY_id(pk))) {
    ...
    case EVP_PKEY_DSA:
      ...
      attrs << pk11_attr_ulong(CKA_KEY_TYPE, CKK_DSA)
            << pk11_attr_data(CKA_PRIME, p) << pk11_attr_data(CKA_SUBPRIME, q)
            << pk11_attr_data(CKA_BASE, g);
      break;
#ifndef OPENSSL_NO_EC
    case EVP_PKEY_EC:
      ec = EVP_PKEY_get0_EC_KEY(pk);
      ba = i2d_bytearray(I2D_VOID(i2d_ECPKParameters), EC_KEY_get0_group(ec));
    ...
    default:
      throw errorEx(
          QString("Unknown Keytype %d").arg(EVP_PKEY_type(EVP_PKEY_id(pk))));
  }
  return attrs;
}
...
void pki_scard::store_token(const slotid& slot, EVP_PKEY* pkey) {
  ...
  pub_atts << new_id << pk11_attr_bool(CKA_TOKEN, true)
  ...
  priv_atts << new_id << pk11_attr_bool(CKA_TOKEN, true)
  ...
  switch (EVP_PKEY_type(EVP_PKEY_id(pkey))) {
    case EVP_PKEY_RSA:
      ...
      priv_atts << pk11_attr_data(CKA_PRIVATE_EXPONENT, d)
    ...
    case EVP_PKEY_EC: {
      ...
      point =
          EC_POINT_point2bn(EC_KEY_get0_group(ec), EC_KEY_get0_public_key(ec),
      ...
      /* Private key */
      priv_atts << pk11_attr_data(CKA_VALUE, EC_KEY_get0_private_key(ec));
      break;
    }
#endif
    default:
      throw errorEx(QString("Unknown Keytype %d").arg(EVP_PKEY_id(pkey)));
  }
...
QList<int> pki_scard::possibleHashNids() {
  ...
  if (nids.count() == 0) {
    switch (EVP_PKEY_type(getKeyType())) {
      case EVP_PKEY_RSA:
        nids << NID_md5 << NID_sha1 << NID_sha256 << NID_sha384 << NID_sha512
             << NID_ripemd160;
        break;
      case EVP_PKEY_DSA:
#ifndef OPENSSL_NO_EC
...
class keygenThread final : public QThread {
 public:
  ...
  void run() final {
    try {
      id = p11->generateKey(name, task.ktype.mech, task.size, task.ec_nid);
    } catch (errorEx& e) {
      err = e;
    }
  }
...
int pki_scard::verify() { return true; }

bool pki_scard::isToken() { return true; }

QVariant pki_scard::getIcon(const dbheader* hd) const {
  return hd->id == HD_internal_name ? QVariant(QPixmap(":scardIco"))
                                    : QVariant();
}

#include "pkcs_11.h"  // <---------------------|
// TODO(melg): fix includes depends on order -|
...
void pk11_attlist::copy(const pk11_attlist& a) {
  ...
  if (alloc_len) {
    attributes = (CK_ATTRIBUTE*)malloc(alloc_len * sizeof(*attributes));
...
pk11_attlist::pk11_attlist(const pk11_attlist& a) { copy(a); }
...
void pk11_attlist::addAttribute(const pk11_attribute& a) {
  ...
  if (attlen == alloc_len) {
    ...
    attributes =
        (CK_ATTRIBUTE*)realloc(attributes, alloc_len * sizeof(*attributes));
    Q_CHECK_PTR(attributes);
  }

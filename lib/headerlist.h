...
class dbheader {
 ...
 public:
  ...
  bool mustSave() {
    return size != -1 || visualIndex != -1 || sortIndicator != -1 ||
           show != showDefault;
  }
  bool operator==(const dbheader* h) const {
    if (h->id == HD_undef) return name == h->name;
    return id == h->id;
  }
  bool isNumeric() {
    switch (id) {
  ...
  QString toData() {
    QStringList sl;
    sl << QString::number(visualIndex) << QString::number(sortIndicator)
       << QString::number(size) << QString::number(show);
    return sl.join(" ");
  }
  void fromData(QString s) {
    QStringList sl = s.split(" ");
    if (sl.count() != 4) {
      qCritical() << "Invalid header data for" << id << name << s;
      return;
    }
  ...
  void setupHeaderView(int sect, QHeaderView* hv) {
    ...
    if (size != -1) hv->resizeSection(sect, size);
    if (sortIndicator != -1) {
      hv->setSortIndicator(
          sect, sortIndicator ? Qt::DescendingOrder : Qt::AscendingOrder);
    }
  }
  void reset() {
    action = nullptr;
...
class nid_dbheader final : public dbheader {
 ...
 public:
  ...
  QString getName() final { return Settings["translate_dn"] ? tooltip : name; }
  QString getTooltip() final {
    return QString("[%1] %2").arg(sn).arg(Settings["translate_dn"] ? name
                                                                   : tooltip);
  }
};
...
class key_dbheader : public dbheader {
 public:
  key_dbheader(int aid, QString aname) : dbheader(aid, false, aname) {
    type = hd_key;
  }
};
...
class dbheaderList : public QList<dbheader*> {
 public:
  dbheaderList(dbheader* h) : QList<dbheader*>() { append(h); }
  ...
  QString toData() {
    ...
    for (int i = 0; i < count(); i++) {
      QStringList seq;
      dbheader* h = at(i);
      if (!h->mustSave()) continue;
      seq << QString("%1").arg(h->id);
      if (h->id > 0) {
        seq << OBJ_obj2QString(OBJ_nid2obj(h->id), 1);
      }
      seq << h->toData();
      sl << seq.join(":");
    }

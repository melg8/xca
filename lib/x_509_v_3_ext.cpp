...
x509v3ext::x509v3ext() { ext = X509_EXTENSION_new(); }
...
int x509v3ext::getCritical() const { return X509_EXTENSION_get_critical(ext); }
...
static bool genName2conf(GENERAL_NAME* gen,
                         QString tag,
                         QString* single,
                         QString* sect) {
  unsigned char* p;
  ...
  switch (gen->type) {
    ...
    case GEN_RID:
      *single = QString("RID:%1").arg(obj2SnOid(gen->d.rid));
      return true;
    case GEN_OTHERNAME: {
      ...
      if (asn1TypePrintable(type)) {
        ...
        *single = QString("otherName:%1;FORMAT:HEX,%2")
        ...
        for (int i = 0; i < a->length; i++) {
          *single += QString(":%1").arg((int)(a->data[i]), 2, 16, QChar('0'));
        }
      }
      return true;
    }
...
static bool genNameStack2conf(STACK_OF(GENERAL_NAME) * gens,
                              QString tag,
                              QString* single,
                              QString* sect) {
  int i;
...
bool x509v3ext::parse_ainfo(QString* single, QString* adv) const {
  ...
  for (i = 0; i < sk_ACCESS_DESCRIPTION_num(ainfo); i++) {
    ...
    if (!genName2conf(desc->location, QString("%1_%2").arg(tag).arg(i), &one,
                      &sect)) {
      retval = false;
      break;
    }
...
static const BIT_STRING_BITNAME reason_flags[] = {
    {0, "", "unused"},          {1, "", "keyCompromise"},
    {2, "", "CACompromise"},    {3, "", "affiliationChanged"},
    {4, "", "superseded"},      {5, "", "cessationOfOperation"},
    {6, "", "certificateHold"}, {7, "", "privilegeWithdrawn"},
    {8, "", "AACompromise"},    {-1, nullptr, nullptr}};
...
bool x509v3ext::parse_bitstring(QString* single, QString* adv) const {
  ...
  switch (n) {
    case NID_key_usage:
      bnames = key_usage_type_table;
      break;
    case NID_netscape_cert_type:
      bnames = ns_cert_type_table;
      break;
    default:
      return false;
  }
...
static bool nameConstraint(STACK_OF(GENERAL_SUBTREE) * trees,
  ...
  for (int i = 0; i < sk_GENERAL_SUBTREE_num(trees); i++) {
    ...
    if (!genName2conf(tree->base, QString("%1_%2").arg(tag).arg(i), &one,
                      sect)) {
      return false;
    }
...
X509_EXTENSION* x509v3ext::get() const { return X509_EXTENSION_dup(ext); }

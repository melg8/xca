...
MainWindow::MainWindow() : QMainWindow() {
  ...
  wdList << keyButtons << reqButtons << certButtons << tempButtons
         << crlButtons;
...
void MainWindow::setItemEnabled(bool enable) {
  foreach (QWidget* w, wdList) { w->setEnabled(enable); }
  foreach (QWidget* w, wdMenuList) { w->setEnabled(enable); }
  foreach (QAction* a, acList) { a->setEnabled(enable); }
...
void MainWindow::initToken() {
  ...
  try {
    ...
    if (ti.tokenInitialized()) {
      ...
      p.setDescription(
          tr("Please enter the new SO PIN (PUK) for the token '%1'")
              .arg(slotname) +
          "\n" + ti.pinInfo());
...
void MainWindow::manageToken() {
  ...
  try {
    ...
    if (import_dialog->entries() == 0) {
      ...
      XCA_INFO(tr("The token '%1' did not contain any keys or certificates")
                   .arg(ti.label()));
    } else {
...
enum open_result MainWindow::setup_open_database() {
  ...
  if (defdig.isInsecure()) {
    XCA_WARN(tr("The currently used default hash '%1' is insecure. Please "
                "select at least 'SHA 224' for security reasons.")
                 .arg(defdig.name()));
    setOptions();
  }
  ...
  connect(tempView, SIGNAL(newCert(pki_temp*)), Database.model<db_x509>(),
          SLOT(newCert(pki_temp*)));
  connect(tempView, SIGNAL(newReq(pki_temp*)), Database.model<db_x509req>(),
          SLOT(newItem(pki_temp*)));
...
void MainWindow::exportIndex() {
  exportIndex(
      QFileDialog::getSaveFileName(
          ...
          tr("Certificate Index ( index.txt )") + ";;" + tr("All files ( * )")),
      false);
}
...
void MainWindow::generateDHparam() {
  ...
  try {
    ...
    connect(dhgen, SIGNAL(finished()), this, SLOT(generateDHparamDone()));
  } catch (errorEx& err) {
    XCA_ERROR(err);
  }
}
...
void MainWindow::keyPressEvent(QKeyEvent* e) {
  ...
  foreach (XcaTreeView* v, views) {
    if (v) {
      v->header()->resizeSections(QHeaderView::ResizeToContents);
      v->reset();
    }
  }
  update();
...
void MainWindow::dump_database() {
  QString dirname = QFileDialog::getExistingDirectory(nullptr, XCA_TITLE,
                                                      Settings["workingdir"]);
  try {
...
void MainWindow::default_database() { Database.as_default(); }

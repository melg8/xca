...
enum open_result PwDialogUI::execute(pass_info* p,
                                     Passwd* passwd,
                                     bool write,
                                     bool abort) {
...
PwDialog::PwDialog(pass_info* p, bool write) : QDialog(p->getWidget()) {
  pi = p;
  setupUi(this);
...
void PwDialog::accept() {
  ...
  if (takeHex->isChecked()) {
    ...
    if (ret == -1) {
      XCA_WARN(
          tr("Hex password must only contain the characters '0' - '9' and 'a' "
             "- 'f' and it must consist of an even number of characters"));
      return;
    }
  } else {
    final = pw.toLatin1();
...
void PwDialog::buttonPress(QAbstractButton* but) {
  qDebug() << "buttonBox->standardButton(but)" << buttonBox->buttonRole(but)

...
Options::Options(QWidget* parent) : QDialog(parent) {
  ...
  s << tr("Printable string or UTF8 (default)")
    ...
    << tr("UTF8 strings only (RFC2459)") << tr("All strings");
  mbstring->addItems(s);
...
Options::~Options() { delete searchP11; }
...
int Options::exec() {
  ...
  Settings["cert_expiry"] =
      cert_expiry_num->text() + cert_expiry_unit->currentItemData().toString();
  Settings["ical_expiry"] =
      ical_expiry_num->text() + ical_expiry_unit->currentItemData().toString();
...
void Options::on_addButton_clicked() {
  ...
  fname = QFileDialog::getOpenFileName(this, l.caption, getLibDir(), l.filter);

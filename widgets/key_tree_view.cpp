...
void KeyTreeView::fillContextMenu(QMenu* menu,
  ...
  if (key->isToken()) {
    ...
    menu->addAction(tr("Init PIN with SO PIN (PUK)"), this, SLOT(initPin()));
    menu->addAction(tr("Change SO PIN (PUK)"), this, SLOT(changeSoPin()));
  } else if (subExport) {
...
void KeyTreeView::toToken() {
  ...
  try {
    ...
    QString msg =
        tr("Shall the original key '%1' be replaced by the key on the "
...
ExportDialog* KeyTreeView::exportDialog(const QModelIndexList& indexes) {
  ...
  return new ExportDialog(
      ...
      tr("Private Keys ( *.pem *.der *.pk8 );; "
         "SSH Public Keys ( *.pub )"),
      indexes, QPixmap(key->isToken() ? ":scardImg" : ":keyImg"),

...
void SearchPkcs11::on_search_clicked() {
  ...
  searching = new searchThread(filename->text(), getLibExtensions(),
                               subdirs->isChecked());

  liblist->clear();
  connect(searching, SIGNAL(updateLibs(QString)), this,
          SLOT(updateLibs(QString)));
  connect(searching, SIGNAL(updateCurrFile(QString)), this,
          SLOT(updateCurrFile(QString)));
  connect(searching, SIGNAL(finished()), this, SLOT(finishSearch()));

  connect(search, SIGNAL(clicked()), searching, SLOT(cancelSearch()));
...
searchThread::searchThread(QString _dir,
                           const QStringList _ext,
                           bool _recursive) {
  dirname = _dir;
  ext = _ext;
...
void searchThread::cancelSearch() { keepOnRunning = false; }
...
bool searchThread::checkLib(QString file) {
  ...
  if (qf.open(QIODevice::ReadOnly)) {
    ...
    r = QByteArray::fromRawData((char*)p, siz).indexOf("C_GetFunctionList");
    qf.unmap(p);
    qf.close();
  }
  return r != -1;
...
void searchThread::search(QString mydir) {
  QDir dir = QDir(mydir);
  QStringList files =
      dir.entryList(QStringList(ext), QDir::Files | QDir::Readable);

...
void CertDetail::setX509super(pki_x509super* x) {
  ...
  connect(privKey, SIGNAL(doubleClicked(QString)), this, SLOT(showPubKey()));
  ...
  connect(sigAlgo, SIGNAL(doubleClicked(QString)), MainWindow::getResolver(),
...
void CertDetail::setCert(pki_x509* cert) {
  ...
  try {
    ...
    if (cert->getSigner() == nullptr) {
      ...
      connect(signature, SIGNAL(doubleClicked(QString)), this,
              SLOT(showIssuer()));
    }
    ...
    if (cert->isRevoked()) {
      ...
      dateValid->setText(tr("Revoked at %1").arg(rev.getDate().toPretty()));
      dateValid->setRed();
...
void CertDetail::showCert(QWidget* parent, pki_x509super* x) {
  ...
  if (dlg->exec()) {
    ...
    if (!db) {
      ...
      db->updateItem(x, dlg->descr->text(), dlg->comment->toPlainText());
    }
  }
  delete dlg;
}

CertDetail::~CertDetail() { delete tmpPubKey; }

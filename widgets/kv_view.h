...
class kvDelegate : public QItemDelegate {
 public:
  kvDelegate(QObject* parent) : QItemDelegate(parent) {}
...
class comboDelegate final : public kvDelegate {
 ...
 public:
  comboDelegate(QStringList k, QObject* parent = nullptr) : kvDelegate(parent) {
    keys = k;
  }
...
class kvView final : public QTableView {
 ...
 public:
  ...
  ~kvView() final;
  int rowCount() { return model()->rowCount(QModelIndex()); }
  QStringList getRow(int i) {
    return static_cast<kvmodel*>(model())->getRow(i);
  }
  void addRow(const QStringList& newrow);
  void deleteAllRows() { model()->removeRows(0, rowCount(), QModelIndex()); }

...
void NewX509::setupExplicitDN(NIDlist my_dn_nid = NIDlist()) {
  ...
  nameEdits =
      setupExplicitInputs(my_dn_nid + expl_dn_nid, dnWidget, description, 2);
}
...
NewX509::NewX509(QWidget* parent) : QDialog(parent ? parent : mainwin) {
  ...
  tabnames = QStringList({"wizard_src", "wizard_subject", "wizard_extensions",
                          "wizard_keyusage", "wizard_netscape",
                          "wizard_advanced", "comment"});
  ...
  connect(extDNlist->itemDelegateForColumn(1),
          ...
          SLOT(setupExtDNwidget(const QString&, QLineEdit*)));
  connect(subAltName, SIGNAL(textChanged(const QString&)), this,
          SLOT(checkSubAltName(const QString&)));
  connect(issAltName, SIGNAL(textChanged(const QString&)), this,
          SLOT(checkIssAltName(const QString&)));
  connect(crlDist, SIGNAL(textChanged(const QString&)), this,
          SLOT(checkCrlDist(const QString&)));
  connect(authInfAcc, SIGNAL(textChanged(const QString&)), this,
          SLOT(checkAuthInfAcc(const QString&)));
  connect(keymodel, SIGNAL(keyDone(pki_key*)), this,
          SLOT(newKeyDone(pki_key*)));
  connect(reqmodel, SIGNAL(pkiChanged(pki_base*)), this,
          SLOT(itemChanged(pki_base*)));
  ...
  foreach (int nid, nidWidget.keys()) {
    ...
    if (l) {
      ...
      connect(l, SIGNAL(doubleClicked(QString)), MainWindow::getResolver(),
...
NewX509::~NewX509() { delete ctx_cert; }
...
void NewX509::getReqAttributes(pki_x509req* req) {
  foreach (nameEdit e, attrEdits) { req->addAttribute(e.nid, e.edit->text()); }
}

void NewX509::setReqAttributes(pki_x509req* req) {
  foreach (nameEdit e, attrEdits) { e.edit->setText(req->getAttribute(e.nid)); }
}
...
void NewX509::defineSigner(pki_x509* defcert, bool applyTemp) {
  ...
  if (defcert && defcert->canSign()) {
    if (certList->setCurrentPkiItem(defcert) != -1) {
      ...
      if (applyTemp && defcert->getTemplateSqlId().isValid()) {
...
void NewX509::on_applySubject_clicked() {
  ...
  selfComment(tr("Subject applied from template '%1'").arg(t->comboText()));
}

void NewX509::on_applyExtensions_clicked() {
  pki_temp* t = currentTemplate();
  extensionsFromTemplate(t);
  selfComment(tr("Extensions applied from template '%1'").arg(t->comboText()));
}

void NewX509::on_foreignSignRB_toggled(bool) { switchHashAlgo(); }
...
void NewX509::newKeyDone(pki_key* nkey) {
  ...
  if (nkey) {
    selfComment(tr("New key '%1' created").arg(nkey->comboText()));
...
void NewX509::on_usedKeysToo_toggled(bool) {
  ...
  keyList->clear();
  keyList->insertPkiItems(usedKeysToo->isChecked() ? allKeys : unusedKeys);
  keyList->setCurrentPkiItem(cur);
}

pki_key* NewX509::getSelectedKey() { return keyList->currentPkiItem(); }

pki_x509* NewX509::getSelectedSigner() { return certList->currentPkiItem(); }

pki_x509req* NewX509::getSelectedReq() { return reqList->currentPkiItem(); }
...
void NewX509::setX509name(const x509name& n) {
  ...
  foreach (nameEdit ne, nameEdits) { ne.edit->setText(""); }
...
enum pki_source NewX509::getPkiSource() const { return pkiSource; }
...
void NewX509::accept() {
  ...
  if (fromReqCB->isChecked() && !getSelectedReq()->verify()) {
    gotoTab(0);
    xcaWarningBox msg(this, tr("The verification of the Certificate request "
  ...
  if (description->text().isEmpty() && !fromReqCB->isChecked()) {
    ...
    if (cn.isEmpty()) {
      ...
      xcaWarningBox msg(this,
                        tr("The internal name and the common name are "
  ...
  if (keyList->count() == 0 && keyList->isEnabled() &&
      !fromReqCB->isChecked()) {
    gotoTab(1);
    xcaWarningBox msg(this, tr("There is no Key selected for signing."));
  ...
  if (!unsetDN.isEmpty()) {
    ...
    QString text =
        tr("The following distinguished name entries are empty:\n%1\nthough "
           "you have declared them as mandatory in the options menu.")
            .arg(unsetDN);
  ...
  if ((!signkey || signkey->isPubKey()) && pt != tmpl) {
    ...
    gotoTab(signer ? 0 : 1);
    xcaWarningBox msg(
        this, tr("The key you selected for signing is not a private one."));
  ...
  if (hashAlgo->count() > 0 && hashAlgo->current().isInsecure()) {
    gotoTab(0);
    xcaWarningBox msg(this, tr("The currently selected hash algorithm '%1' is "
  ...
  if (validityBox->isEnabled() && notBefore->getDate() > notAfter->getDate()) {
    gotoTab(2);
    QString text =
        tr("The certificate will be out of date before it becomes valid. You "
           "most probably mixed up both dates.");
    xcaWarningBox msg(this, text);
    msg.addButton(QMessageBox::Ok, tr("Edit dates"));
    msg.addButton(QMessageBox::Close, tr("Abort rollout"));
    msg.addButton(QMessageBox::Apply, tr("Continue rollout"));
  ...
  if (r) {
    ...
    if (r == 1) {
      text =
          tr("The certificate contains invalid or duplicate extensions. Check "
             "the validation on the advanced tab.");
      gotoTab(5);
    } else {
      text =
          tr("The certificate contains no extensions. You may apply the "
             "extensions of one of the templates to define the purpose of the "
             "certificate.");
      gotoTab(0);
    }
    xcaWarningBox msg(this, text);
  ...
  if (cn.isEmpty() && san.contains("DNS:copycn") && pt != tmpl) {
    gotoTab(2);
    xcaWarningBox msg(
        this, tr("The subject alternative name shall contain a copy of the "
                 "common name. However, the common name is empty."));
    msg.addButton(QMessageBox::Ok, tr("Edit extensions"));
    msg.addButton(QMessageBox::Close, tr("Abort rollout"));
    msg.addButton(QMessageBox::Apply, tr("Continue rollout"));
    switch (msg.exec()) {

...
Validity::Validity(QWidget* parent) : QDateTimeEdit(parent) {
  ...
  setNow();
  hideTime(false);
  connect(this, SIGNAL(timeChanged(const QTime&)), this,
          SLOT(setMyTime(const QTime&)));
  updateFormatString();
}
...
void Validity::updateFormatString() {
  ...
  if (timeSpec() == Qt::UTC || midnight) {
    ...
    format += QString(" '%1'").arg(QTime::currentTime().toString("t"));
  }
  setDisplayFormat(format);
}

void Validity::setDate(const a1time& a) { setDateTime(a); }
...
void Validity::setMyTime(const QTime& time) { mytime = time; }
